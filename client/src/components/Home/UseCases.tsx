...
const UseCases = () => {
    return (
        <div className='space-y-12'>
            <h2 className='text-6xl font-black italic tracking-[-0.20rem] uppercase text-center underline underline-offset-2 decoration-primarygreen bg-primarygreen/20 w-fit mx-auto'>Use Cases</h2>
            <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 w-7/8 mx-auto'>
                {USE_CASES.map((useCase) => (
                    <UseCaseBox key={useCase.title} title={useCase.title} description={useCase.description} />
                ))}
            </div>
        </div>
    )
