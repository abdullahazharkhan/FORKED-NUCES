...
const DRF_LIKES_URL = `${DRF_BASE}/api/interactions/likes/toggle/`;
...
export async function POST(req: NextRequest) {
    ...
    const { project_id } = body as {
        project_id?: string;
    };

    const drfRes = await fetch(DRF_LIKES_URL, {
        method: "POST",
        headers: {
    ...
    if (!drfRes.ok) {
        return NextResponse.json(
            drfBody || { detail: "Failed to toggle like" },
